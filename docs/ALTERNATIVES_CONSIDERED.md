...
|------|------|--------|--------------|--------------|
| **Pulp-based bitmap scan** (`pulp-kernel` feature) | 2026-05-15 | Prototype, not benchmarked | The existing hand-rolled AVX-512 VPOPCNTDQ path already retires one popcount per cycle on Zen 5; we never produced a head-to-head benchmark showing a portable-SIMD rewrite improves on it | `bitmap_pulp_prototype/` (WIP commit `3e14277` in branch history) |
...
|--------------|-------------|--------|
| `bitmap-pulp` | `pulp-kernel`-feature variant of the single-query bitmap scan | Prototype; see Pulp-based bitmap scan above |
