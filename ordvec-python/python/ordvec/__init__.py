...
``search_asymmetric`` and the dense scoring / candidate generator methods
release the GIL during the heavy Rust scan, so other Python threads run
concurrently. ``add`` also releases the GIL while mutating an index, but mutable
index operations must be treated as exclusive. The input arrays are *read in
place* (not copied) for that window — do not mutate an array from another thread
while a call that received it is in progress, or the scan races the write and
